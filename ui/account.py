...
def render_account_page():
    ...
    if st.button("刷新数据", type="primary"):
        st.rerun()  # 新版本的Streamlit使用st.rerun()替代st.experimental_rerun()
    ...
    try:
        ...
        if balance:
            ...
            st.warning("无法获取账户余额")
            st.info("如果您只想进行回测，可以忽略此错误，返回交易中心页面进行回测。")

    except Exception as e:
        st.error(f"获取账户余额失败: {str(e)}")
        st.info("如果错误信息包含 'INVALID_KEY'，请检查您的API密钥配置是否正确。如果您只想进行回测，可以忽略此错误。")
